...
#       --name tank500gateway --target-name tank500-tools \
#       --lambda-arn <arn> --role-name tank500-gateway-role \
#       --schema-file tank500-tools-schema.json
#
#   python3 create_gateway.py delete --name tank500gateway
# =============================================================================
...
# 前缀跟随 00-config.sh 的 TANK500_SSM_PREFIX（03-deploy.sh 按同一前缀读取）
SSM_GATEWAY_ARN_PARAM = (
    os.environ.get("TANK500_SSM_PREFIX", "/app/tank500") + "/gateway_arn"
)
...
def ensure_gateway_role(role_name: str, lambda_arn: str) -> str:
    ...
    try:
        iam_client.create_role(
            ...
            Description="Allows AgentCore Gateway to invoke the Tank 500 Tools Lambda",
        )
        print("  Role created.")
        created = True
    ...
    iam_client.put_role_policy(
        RoleName=role_name,
        PolicyName="invoke-tank500-tools-lambda",
...
def create(args):
    ...
    if gateway_id:
        ...
        resp = gateway_client.create_gateway(
            name=args.name,
            description="Tank 500 car-sales agent tools gateway",
            roleArn=role_arn,
            protocolType="MCP",
    ...
    if target_id:
        ...
        resp = gateway_client.create_gateway_target(
            ...
            description="Tank 500 sales tools backed by Lambda",
