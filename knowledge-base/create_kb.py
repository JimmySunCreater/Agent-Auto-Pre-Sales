...
# chunk 取 300 tokens：车辆手册是连续说明文（一个功能的描述常跨几段），
# 128 会把"操作步骤"和"前提条件"切散导致检索到断头内容；太大又会把 WARNING/
# NOTICE 噪音块与正文大量混合召回。300 + 20% overlap 是两者的平衡。
# 手册中的 OCR 噪音（如 "0odification"）与 WARNING/NOTICE 碎块是刻意保留的
# "脏数据"（见 docs/design.md §5.1），评估阶段用它暴露真实检索质量问题。
...
# Fixed Tank 500 agent configuration (overridable via environment variables).
KB_NAME = os.environ.get("TANK500_KB_NAME", "tank500-manual-kb")
KB_DESCRIPTION = "Tank 500 owner's manual KB for the car-sales agent"
EMBEDDING_MODEL = os.environ.get(
    "TANK500_EMBED_MODEL", "amazon.titan-embed-text-v2:0"
)  # default embedding model (1024 dims, must match EMBEDDING_DIMENSION)
INCLUSION_PREFIX = "tank500/"  # KB docs live under this key prefix in the data bucket
DOCS_PATH = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "output"
)  # generated by split_manual.py
WORKSHOP_STACK_NAME = os.environ.get(
    "INFRA_STACK_NAME", "workshop-infra"
)  # CFN stack exporting DataBucketName (shared with the HR workshop)
# SSM param storing the KB id（前缀跟随 00-config.sh 的 TANK500_SSM_PREFIX）
SSM_KB_ID_PARAM = (
    os.environ.get("TANK500_SSM_PREFIX", "/app/tank500") + "/knowledge_base_id"
)
...
if __name__ == "__main__":
    ...
    if args.mode == "create":
        # Reuse the CFN-created data bucket; KB docs go under the tank500/ prefix.
