...
execute_tool span（tank500-tools___retrieve_tank500_info 等）:
...
# tank500 的证据类工具：KB 检索 / 竞品档案 / 实时搜索，输出都算 THELMA 的 sources
RETRIEVE_TOOL_MARKERS = ("retrieve_tank500_info", "compare_competitor", "web_search")
...
def _deep_extract_answer(message_str: str) -> str:
    ...
    # 第三层：{"answer":"...","sources":[...]} 或竞品/搜索工具的结构化 JSON
    ...
    if isinstance(lvl3, dict):
        # compare_competitor / web_search 的输出没有 answer 字段，
        # 整个 JSON 载荷即证据文本
...
def extract_turns_for_trace(session_spans: list, target_trace_id: str | None) -> list[dict]:
    ...
    for s in spans:
        if not _is_retrieve_tool(s):
            continue
        # 新版镜像里 "mcp tools/call X" 与 "execute_tool X" 是同一次调用的两层
        if (s.get("name") or "").startswith("mcp tools/call"):
            continue
        query, source_text = "", ""

        # --- 新版托管镜像格式（2026-08）：事件在 s["events"] ---
        for e in (s.get("events") or []):
            if not isinstance(e, dict):
                continue
            attrs = e.get("attributes") or {}
            if e.get("name") == "gen_ai.tool.message":
                q = _safe_json(attrs.get("content"))
                if isinstance(q, dict):
                    if q.get("query"):
                        query = str(q["query"])
                    elif q.get("competitor_name"):
                        query = (
                            f"How does the Tank 500 compare with the "
                            f"{q['competitor_name']}?"
                        )
            elif e.get("name") == "gen_ai.choice" and attrs.get("message"):
                source_text = _deep_extract_answer(attrs["message"])
        ...
        for body in _iter_span_events(s):
            ...
            for m in inp:
                ...
                q = _safe_json(raw)
                if isinstance(q, dict):
                    # 兼容各工具的入参字段；竞品对比需合成语义完整的问题
                    # （trace 无用户原话，裸车名会让 RQC 误判"没回答问题"）
                    if q.get("query"):
                        query = str(q["query"])
                    elif q.get("competitor_name"):
                        query = (
...
def _extract_agent_response(spans: list) -> str:
    ...
    for s in spans:
        ...
        if "chat" not in name and "llm" not in name and "invoke_model" not in name and "strands" not in scope and "invoke_agent" not in name:
            continue
        st = s.get("start_time") or s.get("startTimeUnixNano") or ""

        # --- 新版镜像格式：events[] 里 finish_reason=end_turn 的 gen_ai.choice ---
        for e in (s.get("events") or []):
            if not isinstance(e, dict):
                continue
            attrs = e.get("attributes") or {}
            if e.get("name") == "gen_ai.choice" and attrs.get("finish_reason") == "end_turn":
                raw = attrs.get("message", "")
                parsed = _safe_json(raw)
                if isinstance(parsed, list):
                    txt = " ".join(b.get("text", "") for b in parsed
                                   if isinstance(b, dict) and b.get("text"))
                else:
                    txt = raw if isinstance(raw, str) else ""
                txt = _strip_if_tool_payload(txt.strip())
                if txt:
